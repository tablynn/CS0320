...
public class WeatherHandler implements Route {
  ...
  public String getTemperature(String lat, String lon) throws Exception {
    ...
    // Forecast Moshi
    return parseAPIForecastResponse(forecastResponse.body());
  ...
  /**
   * This helper method parses the API response, taking in forecastResponse.body() as a parameter
   * and returning the temperature of the given location body.
   * @param forecastResponse - the body of the Forecast portion of the HTTP request
   * @return temperature of given coordinates
   * @throws IOException
   */
  public String parseAPIForecastResponse(String forecastResponse) throws IOException {
    Moshi moshiTwo = new Moshi.Builder().build();
    return moshiTwo.adapter(Forecast.class)
        .fromJson(forecastResponse).properties.getTemperature();
  }
